...
![softplus函数](.\images\softplus.jpg)
...
本章关注的问题：寻找神经网络上的一组参数$\theta$，能显著地降低代价函数$J(\theta)$，该代价函数通常包括整个训练集上的性能评估和额外的正则化项。

### 8.1 学习和纯优化有什么不同

学习是间接地优化性能度量P，而纯优化最小化目标$J$本身。

#### 8.1.1 经验风险最小化

机器学习算法的目标是降低下式的期望泛化误差，该数据量被称为**风险**。

> $$
> J^*(\theta)=E_{(x,y)\sim p_{data}}L(f(x;\theta),y)\tag{8.1}
> $$
>
> 这里的期望取自真实的潜在分布$p_{data}$。

用训练集上的经验分布$\hat p(x,y)$替代真实分布$p(x,y)$，则最小化下式这种平均训练误差的训练过程被称为**经验风险最小化**。

> $$
> E_{(x,y)\sim\hat p_{data}}[L(f(x;\theta),y)]=\frac{1}{m}\sum_{i=1}^mL(f(x^{(i)};\theta),y^{(i)})\tag{8.2}
> $$

#### 8.1.2 代理损失函数和提前终止

有时，我们真正关心的损失函数（比如分类误差）并不能被高效优化，这时通常会优化**代理损失函数**（surrogate loss function）。

训练算法的优化与一般优化有一个重要不同：训练算法通常不会停止在局部极小点。

使用提前终止可以在收敛条件满足时停止训练。

#### 8.1.3 批量算法和小批量算法

机器学习与一般优化算法的一点不同是，机器学习算法的目标函数通常可以分解为训练样本上的求和。机器学习在计算参数的每一次更新时通常仅使用整个代价函数中的一部分项来估计代价函数的期望值。

在小数目样本中进行统计估计的两个动机：

1. 以计算梯度为例，准确计算整个训练集上的期望的计算代价是巨大的，因为需要在整个数据集上的每个样本上估计模型。

   > $$
   > \nabla_\theta J(\theta)=E_{x,y\sim\hat p_{data}}\nabla_\theta\log p_{model}(x,y;\theta)\tag{8.3}
   > $$

   > $n$个样本均值的标准差$\sigma/\sqrt{n}$，其中$\sigma$为样本值的真实标准差。分母表明，使用更多样本估计均值的回报是小于线性的。使用100个样本和10000个样本，后者计算量是前者的10000倍，但是均值标准差仅仅降低了10倍。

2. 另一个动机是训练集的冗余。

   > 假想一个最坏的情形，训练集中$m$个样本是相同的，那么使用单个样本得到的估计和使用所有的样本得刀的结果一致，却付出了m倍的时间。

使用整个训练集的优化算法被称为**批量**（batch）或**确定性**（deterministic）梯度算法。

每次只使用单个样本的优化算法被称为**随机**（stochastic）或**在线**（online）算法。“在线”通常指从连续产生样本的数据流中抽取样本的情况，而不是从一个固定大小的训练集中遍历多次采样的情况。

深度学习中的优化算法介于两者之间，使用一个以上而又非全部的训练样本。传统地，称为**小批量**（minibatch）或**小批量随机**（minibatch stochastic）方法。现在简单地称为**随机**（stochastic）方法。

小批量的大小通常由几个因素决定：

1. 更大的批量会计算更精确的梯度估计，但是汇报却小于线性。
2. 极小批量难以充分利用多核架构。
3. 当批量处理中的所有样本可以并行地处理时，内存消耗和批量大小成正比。
4. 在某些硬件上使用特定大小的数组时，运行时间会更少。
5. 可能由于小批量在学习过程中加入了噪声，会产生一些正则化效果。

有些算法对采样误差比其他算法更敏感，有两个可能原因：

1. 使用了很难在少量样本上精确估计的信息。
2. 以放大采样误差的方式使用了信息。

> 小批量是**随机抽取**的同样重要。

实践中每次构建小批量样本时都将样本完全均匀地抽取出来是不太现实的。通常将样本顺序打乱一次，然后按照这个顺序存储起来。之后训练模型时用到的一组组小批量连续样本是固定的，每个独立的模型每次遍历训练数据时都会重复使用这个顺序。这种偏离真实随机采样的方法并不会产生很严重的有害影响。

> 对于小批量随机梯度下降来说，只要没有重复使用样本，它将遵循着真实泛化误差的梯度。

### 8.2 神经网络优化中的挑战

#### 8.2.1 病态

优化凸函数时，最突出的挑战是Hessian矩阵$H$的病态。

适用于其他情况的解决病态的技术并不适用于神经网络。例如，牛顿法解决带有病态条件的Hessian矩阵的凸优化问题时很优秀，但运用到神经网络时需要很大的改动。

#### 8.2.2 局部极小值

对于非凸函数，如神经网络，通常会存在多个局部极小值。

由于**模型可辨识性**（model identifiablity）问题，神经网络和任意具有多个等效参数化潜变量的模型都会具有多个局部极小值。

带有潜变量的模型通常是不可辨认的，因为通过相互交换潜变量，能得到等价的模型。

> 例如神经网络的第一层中，交换$i$和$j$单元的传入权重向量和传出权重向量，能得到等价的模型。如果有$m$层，每次$n$个单元，则会有$n!^m$种排列隐藏单元的方式。这种不可辨识性称为**权重空间对称性**（weight space symmetry）。

是否存在大量代价很高的局部极小值，优化算法是否会碰到这些局部极小值，都是尚未解决的公开问题。

一种能够排除局部极小值是主要问题的检测方法是画出梯度范数随时间的变化。如果梯度范数没有缩小到一个微小的值，那么该问题既不是局部极小值，也不是其他形式的临界点。

#### 8.2.3 高原、鞍点和其他平坦区域

对于很多高维非凸函数而言，局部极小值事实上都远小于另一类梯度为零的点：**鞍点**。

可以将鞍点是味代价函数某个横截面上的局部极小点，或者另一个横截面上的局部极大点。

多数随机函数有以下性质：低维空间中，局部极小值很普遍。更高维空间中，局部极小值很罕见，鞍点则很常见。

对于某类函数$f: R^n\rightarrow R$而言，鞍点和局部极小点的数目比率的期望随$n$指数级增长。

许多随机函数的另一个性质是，在代价较低的区间内，Hessian矩阵的特征值为正的可能性更大。意味着局部极小值点具有低代价可能性大于高代价，具有高代价的临界点更可能是鞍点，具有极高代价的临界点就很可能是局部极大值点。

不具非线性的浅层自编码器，只有全局极小值和鞍点。

对于牛顿法来说，鞍点会影响结果。因为牛顿法旨在寻找梯度为零的点。高维空间中鞍点的激增或许解释了神经网络训练中为什么二阶方法无法成功取代梯度下降。

#### 8.2.4 悬崖和梯度爆炸

多层神经网络通常会存在像悬崖一样斜率较大的区域，通常梯度更新会很大程度改变参数值，然后完全跳过这类悬崖结构。

> 出现这种情况时，梯度下降更新可以使参数弹射得很远，使得大量已完成的优化工作变为无用功。

可以使用启发式**梯度截断**来避免这种严重后果。当传统梯度下降算法提议更新很大一步时，启发式梯度截断会干涉来减少步长，从而使其不太可能走出梯度近似为最抖下降方向的悬崖区域。

#### 8.2.5 长期依赖

当计算图变得极深时，神经网络优化算法会面临的一个难题是**长期依赖**问题。由于变深的结构使得模型丧失了学习到先前信息的能力，让优化变得极其困难。

> 深层计算图不仅存在于前馈网络，还存在于循环网络中。并且由于循环网络要在很长时间序列的各个时刻应用相同操作来构建非常深的计算图，同时模型参数共享，造成该问题更加明显。

**梯度消失与爆炸问题**是指计算图上的梯度会因为$\mathrm{diag}(\lambda)^t$大幅度变化。

> 梯度消失使得无法确定参数朝哪个放下那个移动能够改进代价函数。
>
> 梯度爆炸会使学习过程不稳定。

梯度消失与爆炸问题通常出现在循环网络中，前馈网络中没有。

#### 8.2.6 非精确梯度

大多数优化算法的先决条件都要求有精确的梯度或是Hessian。几乎每一个深度学习算法都需要基于采样的估计，至少使用训练样本都小批量来计算梯度。

当目标函数不可解时，通常梯度也难以处理，这时需要近似梯度。

#### 8.2.7 局部和全局结构间的弱对应

大多数优化研究的难点集中于训练是否找到了全局最小点、局部极小点或是鞍点，但在实践中神经网络不会到达任何一种临界点。甚至，临界点不一定存在。

有些时候，即使没有局部极小点和鞍点，仍然不能从局部优化中找到一个良好的代价函数值。

梯度下降和基本上所有的可以有效训练神经网络的学习算法，都是基于局部较小更新。

局部下降的问题：

1. 能定义通向解的路径，但是该路径包含很多次更新，计算代价过大。
2. 当目标函数有一个宽而平的区域，或试图寻求精确临界点时，局部信息不能提供任何指导。
3. 局部移动过于贪心，朝着下坡方向移动，却和所有可行解南辕北辙，或者用舍近求远的方法求解。

无论哪个问题，如果存在一个区域，使得遵循拒不下降便能合理地直接到达某个解，并且能够在该良好区域初始化学习，那么问题就可以避免。

因此，最终的观点是建议在传统优化算法上怎样选择**更佳的初始点**，以此实现目标更实际。

#### 8.2.8 优化的理论限制

1. 任何优化算法都有性能限制。但不影响实践中的应用。
2. 存在某类问题是不可解的，但很难判断一个特定问题是否属于该类。
3. 寻找给定规模的网络的可行解很困难。但是可以设置更多参数，使用更大的网络。
4. 在神经网络训练中，通常不关注某个函数的精确极小点，只关注将其值下降到足够小以获得一个良好的泛化误差。
...
​		从训练集中采包含$m$个样本$\{x^{(1)}, ..., x^{(m)}\}$的小批量，其中$x^{(i)}$对应目标为$y^{(i)}$。
...
SGD算法中一个关键参数是学习率。实践中，有必要随着时间的推移逐渐降低学习率。

一般会线性衰减学习率直到第$\tau$次迭代：

> $$
> \epsilon_k=(1-\alpha)\epsilon_0+\alpha\epsilon_\tau\tag{8.4}
> $$
>
> 其中$\alpha=\frac{k}{\tau}$。在$\tau$步之后，使$\epsilon$保持常数。

初始学习率$\tau_0$的选择很重要：

- $\tau_0$太大，学习曲线将会剧烈振荡，代价函数值通常会明显增加。
- $\tau_0$太小，学习过程会很缓慢。

SGD及相关小批量或更广义的基于梯度优化的在线学习算法的一个重要性质是每一步更新的计算时间不依赖训练样本数目的多寡。

**额外误差**：$J(\theta)-min_\theta J(\theta)$，即当前代价函数超出最低可能代价的量。

#### 8.3.2 动量

动量方法可以加速随机梯度下降的学习速度，特别是处理高曲率、小但一致的梯度，或是带噪声的梯度。

动量算法引入变量$v$，代表参数在参数空间移动的方向和速率，被设为负梯度的指数衰减平均。假设单位质量，则速度向量$v$可以看作动量。更新规则如下：

> $$
> v\leftarrow\alpha v-\epsilon\nabla_\theta(\frac{1}{m}\sum^m_{i=1}L(f(x^{(i)};\theta),y^{(i)}))\tag{8.5}
> $$

> $$
> \theta\leftarrow\theta+v \tag{8.6}
> $$

### 8.4 参数初始化策略

深度学习模型的训练算法通常是迭代的并要求指定一些开始迭代的点。

初始点对算法的影响：

1. 初始点的选择能够决定算法是否收敛。
2. 当学习收敛时，初始点可以决定学习收敛得速度，以及是否收敛到一个代价高或低的点。
3. 初始点还可能影响泛化。

现代初始化策略是简单的、启发式的。

> 唯一确定的特性是初始参数需要在不同单元间“破坏对称性”。即如果具有相同激活函数的两个隐藏单元连接到相同的输入，那么这些单元必须具有不同的初始参数。

通常，可以为每个单元的偏置设置启发式挑选的常数，仅随机初始化权重。

权重的理想初始大小由以下因素决定：

1. 更大的初始权重具有更强的破坏对称性的作用，有助于避免冗余的单元。
2. 更大的初始权重还有助于避免在每层线性成分的前向或反向传播中丢失信号。
3. 初始权重过大会在前向传播或反向传播中产生爆炸的值。
4. 在循环网络中，很大的权重可能导致**混沌**（对于输入中很小的扰动非常敏感，导致确定性前向传播过程表现随机）。
5. 较大的权重会产生使得激活函数饱和的值，导致饱和单元的梯度完全丢失。

选择权重初始大小的启发式方法：

1. 假设为$m$个输入$n$个输出的全连接层，可以从分布$U(-\frac{1}{\sqrt m},\frac{1}{\sqrt m})$采样权重。
2. 初始化为随机正交矩阵，挑选每一层特定的非线性缩放或增益因子$g$。
3. 仅正确设置缩放因子足以训练深达1000层的网络，无需使用正交初始化。

> 实践中，通常将权重范围视为超参数。

数值范围准则的一个缺点：设置所有的初始权重具有相同的标准差，会使层很大时每个单一权重变得极其小。

> 针对这个问题，有人提出稀疏初始化：每个单元初始化恰好有$k$个非零权重。

当计算资源允许时，可以将每层权重的初始数值范围设为超参数。

选择初始数值范围的经验法则是观测单个小批量数据上的激活或梯度梯度的幅度或标准差。如果权重太小，当激活值在小批量上前向传播于网络时，激活值的幅度会缩小。通过重复识别具有小得不可接受的激活值的第一层，并提高其权重，最终可能得到一个初始激活全部合理的网络。

设置偏置必须与设置权重的方法协调，通常可设置偏置为零。设置偏置为非零值的情况有：
...
​		计算更新：$\Delta\theta\leftarrow-\frac{\epsilon}{\delta+\sqrt{r}}\bigodot g$
...
Require：小常数$\delta$，为了数值稳定大约设为$10^{-6}$（用于被小数除时的数值稳定）
...
​		累积平方梯度：$r\leftarrow \rho r+(1-\rho)g\bigodot g$

​		计算更新：$\Delta\theta\leftarrow-\frac{\epsilon}{\delta+\sqrt{r}}\bigodot g$（$\frac{\epsilon}{\delta+\sqrt{r}}$逐元素应用）
...
​		计算临时更新：$\widetilde\theta\leftarrow\theta+\alpha v$

​		计算梯度：$g\leftarrow \frac{1}{m}\nabla_\widetilde\theta\sum_iL(f(x^{(i)};\widetilde\theta), y^{(i)})$

​		累积平方梯度：$r\leftarrow \rho r+(1-\rho)g\bigodot g$

​		计算更新：$v\leftarrow\alpha v-\frac{\epsilon}{\sqrt r}\bigodot g$（$\frac{1}{\sqrt r}$逐元素应用）
...
​		更新有偏二阶矩估计：$r\leftarrow\rho_2r+(1-\rho_2)g\bigodot g$

​		修正一阶矩的偏差：$\widetilde s\leftarrow\frac{s}{1-\rho_1^t}$

​		修正二阶矩的偏差：$\widetilde r\leftarrow\frac{r}{1-\rho_2^t}$

​		计算更新：$\Delta\theta=-\epsilon\frac{\widetilde s}{\sqrt{\widetilde r}+\delta}$
...
使用率高的优化算法包括：SGD、具动量的SGD、RMSProp、具动量的RMSPror、AdaDelta和Adam。

选择哪个算法取决于对算法的熟悉程度，以便调节超参数。

### 8.6 二阶近似方法

#### 8.6.1 牛顿法

牛顿法基于二阶泰勒级数展开在某点$\theta_0$附近来近似$J(\theta)$的优化方法，忽略了高阶倒数：

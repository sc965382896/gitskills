...
2. 默认参数，传入参数的数目可与定义时不一致，会出现三种情况
	1. 按照顺序输入参数，缺失的参数按照默认值；
	2. 使用关键字输入参数，缺失的参数按照默认值；
	3. 默认参数之后有可变参数时，输入参数个数大于等于位置参数和默认参数之和时，会按照关键字或顺序传入默认参数，而不使用默认值，当输入参数少于位置参数和默认参数时，缺失的参数会使用默认值。
	> 默认参数必须指向不变对象。
...
* zip()接收多个Iterable，返回Iterator。功能是将多个序列作为元组的元素缝合在一起，序列的元素个数可以不一致，以最短的为准。
...
* reversed()接收一个序列对其进行反转，不会改变原序列，返回一个反转后的序列。
